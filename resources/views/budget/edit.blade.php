        ...
        <div class="container">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-9">
                            <h4>Creacion de presupuestos</h4>
                        </div>
                        <div class="col-md-3">
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('budget') }}" class="btn btn-primary font-weight-bold">Volver</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <form name="provider" id="provider" method="post" action="{{route( 'budget.update' )}}">
                        <input type="hidden" name="id" value="{{$budget->id}}">
                        <div class=" col-md-6 mb-3">
                            <label for="date">Fecha</label>
                            <input type="date" class="form-control" date-format="mm/dd/yyyy"
                                id="date" name="date" placeholder="" value="{{$budget->fecha}}">
                            ...
                            <select class="form-control" name="AD_Org_ID" id="AD_Org_ID">
                                @if (isset($orgs))
                                    @if ($orgs)
                                        @foreach ($orgs as $org)
                                            @if($org->id!=0)
                                                <option value="{{ $org->Name }}" {{ $org->Name == $budget->sucursal ? 'selected' : '' }}>{{ $org->Name }}</option>
                                            @endif
                                        @endforeach
                                    @endif
                                @endif
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="recibe" name="recibe" value="{{$budget->responsable_recibe}}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="entrega">Responsable que entrega </label>
                            <input type="text" class="form-control" id="entrega" name="entrega" value="{{$budget->responsable_entrega}}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="presupuesto">Presupuesto </label>
                            <input type="number" class="form-control" id="presupuesto" name="presupuesto" value="{{$budget->presupuesto}}"  step="0.01" required>
                        </div>
                        <div class="col-md-6 mb-3">
